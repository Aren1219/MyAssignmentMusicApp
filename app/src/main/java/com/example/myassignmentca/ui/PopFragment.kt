...
import com.example.myassignmentca.databinding.FragmentPopBinding
import com.example.myassignmentca.util.UiState
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class PopFragment : Fragment(R.layout.fragment_classic) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val viewModel by lazy {
            ViewModelProvider(requireActivity())[PopViewModel::class.java]
        }
        binding = FragmentClassicBinding.bind(view)

        binding.rvMusic.apply {
            background = AppCompatResources.getDrawable(context, R.color.bgPopMusic)
            layoutManager = LinearLayoutManager(context)
//            adapter = MusicAdapter(listOf())
        }

        binding.srMusic.setOnRefreshListener {
            viewModel.getPopMusic()
        }
